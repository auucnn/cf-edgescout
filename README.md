...
掃描流程會同時查詢多個網段提供方（`providers` 參數），系統內建官方 Cloudflare 公布的清單以及常見的第三方社群鏡像。當任一提供方暫時失效時，程式會記錄警告訊息並嘗試使用其他來源；若所有提供方均不可用，則會退回到快取的聚合資料（`sources`/`cache-dir` 設定），確保仍能進行採樣。此機制也適用於常駐模式 (`daemon`)，無須重新啟動即可自動恢復資料。
